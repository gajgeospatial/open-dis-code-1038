...
public class WebSocketServer 
{
    ...
    public static final int HTTPS_WEBSERVER_PORT = 443;
    ...
    public static void main(String[] args) throws Exception 
    {
        ...
        httpConnector.setPort(webserverPort);
        
        // Set up https connection
        // get the https port
        int httpsWebserverPort = HTTPS_WEBSERVER_PORT;

        try
        {
            httpsWebserverPort = Integer.parseInt(config.getProperty("httpsWebserverPort"));
        }
        catch(Exception e)
        {
            System.out.println("webserver port not specified in GatewayConfiguration.properties, using default of " + DEFAULT_WEBSERVER_PORT);
        ...
        httpsConnector.setPort(httpsWebserverPort);
        ...
        // We can do caching. Turn that off for now.
       ...
       // it finds a match.
        ...
        aThread.start();
        System.out.println("Started DIS listening on UDP port " + port);
       ...
       */
        ...
        System.out.println("Starting https server on TCP port " + httpsWebserverPort);
        server.start();
        server.join();

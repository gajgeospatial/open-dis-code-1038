...
int main(int argc, char* argv[])
{
   ...
   std::string ip("224.224.224.224");
   bool from_file = true;
   std::string filename = "C:\\temp\\disonly.pcap";
   if (argc > 2)
   {
   ...
   while( true )
   {
	  ...
	  int offset = 0;
	  int size2send = 0;
	  char filebuffer[Example::MTU_SIZE + 1];
	  int bytes_read = 0;
	  int sendcnt = 0;
	  if (from_file)
	  {
		  struct pcap_pkthdr h;
		  filebuffer[Example::MTU_SIZE] = '\0';   // should be NULL terminated somewhere in case the network read does not do this?

		  int status = pcap_next_packet(fp, &h, (u_char *)filebuffer, Example::MTU_SIZE);
	  ...
	  if (from_file)
	  {
		  if (bytes_read > 0)
		  {
			  DIS::DataStream ds(filebuffer + 42, bytes_read, endian);
			  multicast.Send(&ds[0], bytes_read);
			  ++sendcnt;
			  printf(".");
			  if ((sendcnt % 40) == 0)
				  printf("\n");

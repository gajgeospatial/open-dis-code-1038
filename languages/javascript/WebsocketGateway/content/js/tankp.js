...
function TankP(loader, scene, colliders, overlay ) 
{
  this.mesh;
  this.killed = false;
  ...
  */
  var onGeometry = function(geom, mats) {
    ...
    scene.add(that.mesh);
	colliders.push(that.mesh);
	that.mesh.add(overlay.sp);
    that.mesh.add(that.psys);
	that.mesh.position.set(0,-40,0);
    loader.load("models/hovertank2/turret.js", onGeometryTurret);
  };
  ...
  var onGeometryTurret = function(geom, mats) {
	...
	colliders.push(that.turretMesh);
    loader.load("models/hovertank2/barrel.js", onGeometryBarrel);
  };
  var onGeometryBarrel = function(geom, mats) {
    ...
    loader.load("models/hovertank2/muzzleflash.js", onGeometryMuzzleflash);
  };
  ...
  loader.load("models/hovertank2/hovertank10.js", onGeometry);

  
  
  
  scene.add(overlay.sp)
  ...
  this.update = function(dt,now) {
    ...
    var rayNegY = new THREE.Raycaster( this.mesh.position({y:y+2}), yUnitNeg );
    ...
    if (collisionsPosY.length>0) {
      this.mesh.position.setY( collisionsPosY[0].point.y +0.5);
    }	

...
{
    /** Experimental. The theory is that if we have N cores on the
     * CPU, we can use N-1 distribution threads and take advantage
     * of parallelism. There are still problems with contention
     * on the FIFO queue, though.
     * ETA: Experiment failed! It turns out the Java Websocket RemoteEndpoint
     * object is not thread-safe, so having two threads sending to the
     * same websocket endpoint will fail. This MUST be set to 1!
     */
    public static final int NUMBER_OF_DISTRUBTION_THREADS = 1;
    ...
    public static Thread[] distributionThreadPool;
    ...
    public static synchronized ConnectionManager getConnectionManager()
    {
        if(connectionManager == null)
        {
            ...
            distributionThreadPool = new Thread[NUMBER_OF_DISTRUBTION_THREADS];
            
            for(int idx = 0; idx < NUMBER_OF_DISTRUBTION_THREADS; idx++)
            {
                distributionThreadPool[idx] = new Thread(new MessageDistributor(connectionManager));
                // Start the thread that repeats an incoming message to clients
                distributionThreadPool[idx].start();
            }
